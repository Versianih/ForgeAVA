...
from brain.gerador import call_gemini, save_file, read_prompt
...
class ForgeAVA:
    ...
    def show_generate_ava_code(self):
        ...
        # Botão "Gerar Código"
        self.code_button = tk.Button(generate_code_frame, text="Gerar Código", command=self.botao_codigo_ava, bg="grey", fg="black", state="disabled")
    ...
    def gerar_codigo_ava(self):
        ...
        if self.file_option_selected.get() == "enviar":
            ...
            if self.ava_option_selected.get() == "url":
                ...
                if send_file_to_ava(login=self.LOGIN, password=self.SENHA, id=self.id_url_activity, file_path=self.PASTA_SAIDA, file=self.file.get()) == "error":
                    self.ava_loading_message.config(text="")
                ...
                # Tenta enviar arquivo para o AVA
                if send_file_to_ava(login=self.LOGIN, password=self.SENHA, id=self.id_or_url_activity.get(), file_path=self.PASTA_SAIDA, file=self.file.get()) == "error":
                    self.ava_loading_message.config(text="")
                    print("ERRO ao enviar arquivo para o AVA")
                    messagebox.showinfo("Gerador AVA", "ERRO ao enviar arquivo para o AVA")
                    self.root.after(3, self.show_generate_ava_code)
    ...
    def open_file(self):
        ...
        try:
            ...
            self.editing_area.bind('<Tab>', self.inserirEspaço)

            # Frame Botões
            buttons_frame = tk.Frame(editing_frame)
            buttons_frame.pack(fill="x", pady=10, padx=10, anchor="e")
            
            save_button = tk.Button(buttons_frame, text="Salvar Alterações", command=lambda: self.salvarEdições(self.selected_file), fg="white", bg="green")
    ...
    def salvarEdições(self, nome_arquivo):
        ...
        file_path = os.path.join(self.PASTA_SAIDA, nome_arquivo)
    ...
    def show_settings(self):
        ...
        path_frame = tk.Frame(settings_frame, relief="solid", borderwidth=1, padx=5, pady=5)
        path_frame.pack(fill="x", pady=5)
        tk.Label(path_frame, text="Pasta de Saída:").pack(side="left")
        self.output_folder_label = tk.Label(path_frame, text=self.PASTA_SAIDA if self.PASTA_SAIDA else "Nenhuma pasta selecionada", fg="green" if self.PASTA_SAIDA else "red")
        self.output_folder_label.pack(side="left")

        tk.Button(path_frame, text="Selecionar Outra Pasta" if self.PASTA_SAIDA else "Selecionar Pasta", command=self.selecionar_pasta).pack(pady=10, side="right")
    ...
    def open_prompt(self):
        ...
        save_button = tk.Button(button_frame, text="Salvar Alterações", command=lambda: self.save_prompt_edition(self.prompt_path), fg="white", bg="green")
        save_button.pack(side="right", padx=5)
